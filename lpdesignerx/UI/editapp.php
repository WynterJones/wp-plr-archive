...
<div class="editTop">
	...
	<div style="float: right; margin-top: 15px; margin-right: -60px">
		<?php 
			if (get_option($_GET['id'].'_lp_views') != '') {
				$views = get_option($_GET['id'].'_lp_views');	
			}
			else {
				$views = 0;
			}
			if (get_option('clicks_lp'.'_'.$_GET['id']) != '') {
				$clicks = get_option('clicks_lp'.'_'.$_GET['id']) ;
			}
			else {
				$clicks = 0;
			}
...
<div class="editNav">
		...
		<div class="editItem2" style="float:right;">
			<span class="grey-btn btn" id="saveIt" style="margin-left:-25px;" ><a href="#" id="saveIt_text">Update Page</a></span>
		</div>
		<br clear="all" >
</div>
<div class="editArea">
