<?php
	...
	$assets = $full_path . "/wp-content/plugins/wpoptinlock/lp/";
	// Get Full Path - echo $assets;
	// Track Views
	  $views = get_option($ID.'_optinviews');
	  update_option($ID.'_optinviews', $views + 1);
?>
<!DOCTYPE html>
<html lang="en">
  <head>
	    ...
	    <script type="text/javascript">
	      $(document).ready(function() {
		      ...
		      $('form').submit(function() {
		           ...
		           $('div.LockedContent').fadeIn();

		           $.cookie('wpoptinlock_<?php echo $_GET["id"]; ?>', 'signedup32223');

		      });

		       if($.cookie('wpoptinlock_<?php echo $_GET["id"]; ?>') == "signedup32223") {
			$('div.mainContainer').hide();
		           $('div.LockedContent').fadeIn();
		       }
...
<body>
	<div class="row" style="padding: 0">
		...
		<div class="container LockedContent" style="display: none; padding: 0">
			<?php echo stripslashes($results->lockedContent); ?>
		</div>
		<div class="container mainContainer" >
			...
			<form action="" method="post" class="ARform">
				<div class="col-xs-6" style="padding: 0; padding-right: 5px;">
					...
					<input required type="text" placeholder="Your Full Name" class="form-control"  id="optinName" />
				</div>
				<div class="col-xs-6" style="padding: 0; padding-left: 5px;">
					<strong><i class="fa fa-envelope"></i> Your Email Address:</strong>
					<input required type="email" <?php if($results->ar == "mailchimp") { echo 'name="EMAIL" '; } else { echo 'name="email" '; } ?> placeholder="Your Email Address" class="form-control"  id="optinEmail" />
				</div>
				<div class="col-xs-12" style="padding: 0">
					<input type="submit" class="btn btn-primary btn-lg btn-block trackClick" id="signUpNow" value="Click Here to Sign Up Now" />
