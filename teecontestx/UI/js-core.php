...
jQuery(document).ready(function($) {
	...
	$('.saveIt').click(function() {
		$('.saveIt').attr('class', 'saved btn saveIt');
		setTimeout(function(){
			$('.saveIt').attr('class', 'grey-btn btn saveIt');
		},300);
	...
	$('#cloneIt').click(function() {
		$('#cloneIt').attr('class', 'saved btn');
		setTimeout(function(){
			$('#cloneIt').attr('class', 'grey-btn btn');
		},300)
	    // Loop Through all WP Editors
	    $( ".wp-editor-wrap" ).each(function( index ) {
	    	$getID = $('#cloneIt').attr("theid"); // get ID wp-ID-wrap
		  	$getID = $getID.replace("wp-", ""); // replace pre-fix
		  	$getID = $getID.replace("-wrap", ""); // replace post-fix
		  	if ($("#wp-"+$getID+"-wrap").hasClass("tmce-active")){  // on Visual State
	        	$getContent = tinyMCE.get($getID).getContent();
			...
			var url = $('#url').val();
		var clone = $('#clone').val();
		window.location.href = url+'id='+clone;
        });
		// setTimeout('wpfix()', 1000);
	  return false;
	});
	// Clone Page Link 
	$('#cloneThePage').click(function() {
		var url = $('#url').val();
		var clone = $('#clone').val();
		var to = $('#to').val(); 
		window.location.href = url+'clone='+clone+'&to='+to;
	});
