...
<div class="editTop">
	...
	<div style="float: right; margin-top: 15px; margin-right: -60px">
		<?php
			if (get_option($_GET['id'].'optinremix_views') != '') {
				$views = get_option($_GET['id'].'optinremix_views');
			}
			else {
				$views = 0;
			}
			if (get_option('clicks_optinRemix'.'_'.$_GET['id']) != '') {
				$clicks = get_option('clicks_optinRemix'.'_'.$_GET['id']) ;
			}
			else {
				$clicks = 0;
			}
